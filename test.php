...
$sid = 'st-106';
// http://localhost/h_st/student/instructions_exam.php?e=24&s=geo&ex_n=test
...
<div class="mcqs">
   ...
   <?php
if($st_result_num == 0){
    $mcq_fetch = "SELECT * FROM exam_data e RIGHT JOIN mcq a on a.exam_name = '$url_exam_name' and a.exam_id = e.exam_id where a.class = '$class' and a.section = '$sec' and e.class = '$class' and e.section = '$sec' and e.status = 'active' and e.status <> 'done' and e.subject = '$url_subject'";
    ...
    while($mcq_fetch = mysqli_fetch_assoc($result_fetch)){
        ...
        $subject = $mcq_fetch['subject'];
            echo "<div id='mdiv-$js_id' class='container mt-sm-5 my-1'>
            <div id='marks'> marks: $m_marks </div>
            <div class='question ml-sm-5 pl-sm-5 pt-2'>
                <div id='heading' class='py-2 h5'><h4>Q;$js_id:- $m_title</h4></div>
                <div class='ml-md-3 ml-sm-3 pl-md-5 pt-sm-0 pt-3' id='options'> 
                <input type='hidden' id='mcq_que-$js_id' value='"."$m_mcq_id"."'>
                <label class='options'>$m_option_1 <input type='radio' id='ra-$js_id' name='ans_mcq' value='$m_option_1' > <span class='checkmark'></span> </label>
                <label class='options'>$m_option_2 <input type='radio' id='ra-$js_id' name='ans_mcq'value='$m_option_2' > <span class='checkmark'></span> </label>
                <label class='options'>$m_option_3 <input type='radio' id='ra-$js_id' name='ans_mcq' value='$m_option_3' > <span class='checkmark'></span> </label>
                <label class='options'>$m_option_4 <input type='radio' id='ra-$js_id' name='ans_mcq' value='$m_option_4' > <span class='checkmark'></span> </label> </div>
            </div>
            <div class='d-flex align-items-center pt-3'>
                <div class='ml-auto mr-sm-5'>
                <input id='mcq-$js_id' type='submit' class='btn' value='Submit MCQ'>
                </div>
            </div>
        </div>";
        $js_id = $js_id+1;
...
<?php
if($st_result_num == 0){
    $que_fetch = "SELECT * FROM exam_data e RIGHT JOIN questions a on a.exam_id = e.exam_id where a.exam_name = '$url_exam_name'and a.class = '$class' and a.section = '$sec' and e.class = '$class' and e.section = '$sec' and e.status = 'active' and e.status <> 'done' and e.subject = '$url_subject'";
    ...
    while($question_fetch = mysqli_fetch_assoc($exam_que_fetch)){
        ...
        echo "<div id='div-$js_id_que' class='question_form'>
        <div id='marks'> marks: $marks </div>
        <input type='hidden' id='que_id-$js_id_que' value='$q_id'>
        <div id='heading' class='py-2 h5'><h4>Q;$js_id_que:- $question</h4></div>
            <h2>Answer Here:</h2>
            <textarea id='txt-$js_id_que' onpaste='paste()' class='data_st' name='ans_q' placeholder='Answer Here' rows='10' cols='65' >

            </textarea>
            <br>
            <input id='que-$js_id_que' type='submit' class='btn_q' value='Submit Quetion'>
        </div>";
...
<script>
// -------------- start of remove Que /Mcqs logic
// for exams

// var x = document.getElementById("que").rows.length
x = 2;
// console.log(x);
// for questions
var que='que-';// btn
var que_row='div-'; // div
var txt='txt-'; // ans
var in_que_id='que_id-'; // que_id
for (let i=1; i<=x; i++) {
    this["que_id"+i]  = que+i;  //submit btn
    this["que_row_id"+i]  = que_row+i;  // Div
    this["txt_id"+i]  = txt+i;  // txt id (ans of textarea)
    this["in_que_id_sub"+i]  = in_que_id+i;  // que id (Quetion Id)

    this["que_submit"+i] = document.getElementById(this["que_id"+i]); //Btn
    this["que_submit_row"+i] = document.getElementById(this["que_row_id"+i]); // Div
    this["txt_id_element"+i] = document.getElementById(this["txt_id"+i]); // txt element
    this["que_id_element_sub"+i] = document.getElementById(this["in_que_id_sub"+i]); // ans element
// console.log('kk')
//     console.log(this["que_submit"+i]);
//     console.log(this["que_submit_row"+i]);
//     console.log(this["txt_id_element"+i]);

// call ajax and hide it
	this["que_submit"+i].addEventListener('click',()=>{
        this["que_submit_row"+i].style.display = 'none';

        var ans_que = this["txt_id_element"+i].value;
        var que_id_ajax = this["que_id_element_sub"+i].value;


// console.log('ID IS => '+que_id_ajax)
// console.log(ans_que)

        $.ajax({
        url: "student/que_submit.php",
        type: "POST",
        data: {ans:ans_que, roll_no:rollNo, que_id:que_id_ajax, tid:tId, st_name:name, exam_id:examId, sub:url_sub, exam_name:url_exam_name},
        success: function(data){
            if(data == 1){
        }else if (data == 0){
            alert('System Error Contact Admin');
...
for (let i=1; i<=x; i++) {
    this["mcq_id"+i] = mcq+i;  //Btn id
    this["mcq_row_id"+i] = mcq_row+i;  // Div id
    this["mcq_que_element"+i] = mcq_que+i;  // ans id element

    // var mcq_ans = ans+i;  //ans

    this["mcq_submit"+i]  = document.getElementById(this["mcq_id"+i]); //submit id
    this["mcq_submit_row"+i]  = document.getElementById(this["mcq_row_id"+i]); // row id
    this["mcq_que_id_ajax"+i]  = document.getElementById(this["mcq_que_element"+i]) // ans_que id element

    console.log(this["mcq_submit"+i])
    console.log(this["mcq_submit_row"+i])

	this["mcq_submit"+i].addEventListener('click',()=>{
    this["mcq_submit_row"+i].style.display = 'none';
    var ajax_and_que_id = this["mcq_que_id_ajax"+i].value;
    var selectedOption = $("input:radio[name=ans_mcq]:checked").val();
    console.log(ajax_and_que_id);
    console.log(selectedOption);

    $.ajax({
        url: "student/mcq_submit.php",
        type: "POST",
        data: {ans:selectedOption, roll_no:rollNo, que_id:ajax_and_que_id, tid:tId, st_name:name, exam_id:examId, sub:url_sub, exam_name:url_exam_name},
        success: function(data){
            if(data == 1){
        }else if (data == 0){
            alert('System Error Contact Admin');
