...
// End of profile_img
if (isset($_POST['open_t'])) {
  $id = $_POST['id'];
  header('location:teacher_pro.php?t='.$id.'');
}
if (isset($_POST['app_t'])) {
  $id = $_POST['id'];
  $sql="UPDATE `teachers` SET `acc_approval` = '1' WHERE `t_id` = '$id';";
  $result = mysqli_query($conn,$sql);

  if($result){
  header('location:teachelist.php?err=ad&c='.$url_class.'');
  }else{
    header('location:teachelist.php?err=af&=c='.$url_class.'');
  }
}

if (isset($_POST['app_b'])) {
  $id = $_POST['id'];
  $sql="UPDATE `teachers` SET `acc_approval` = '0' WHERE `t_id` = '$id';";
  $result = mysqli_query($conn,$sql);

  if($result){
  header('location:teachelist.php?err=bd&c='.$url_class.'');
  }else{
  header('location:teachelist.php?err=bf&c='.$url_class.'');
  }
}
?>
<!DOCTYPE html>
...
?>
  ...
  <?php
    ...
    if(isset($_GET['err']) && $_GET['err'] == "ad"){
        ...
        <strong>Success!</strong>  Account Active Succesfully 
        </div>";
    }else if (isset($_GET['err']) && $_GET['err'] == "af"){
        echo  "<div id='mes' class='alert alert-danger alert-white rounded'>
        <button type='button' class='close' data-dismiss='alert' aria-hidden='true' id='cut'>×</button>
        <div class='icon'><i class='fa fa-times-circle'></i></div>
        <strong>Error!</strong> Account Active UnSuccesfull, Contact Admin
        </div>";}
    if(isset($_GET['err']) && $_GET['err'] == "bd"){
        echo  "<div id='mes' class='alert alert-success alert-white rounded'>
                <button type='button' class='close' data-dismiss='alert' aria-hidden='true' id='cut'>×</button>
                <div class='icon'><i class='fa fa-check'></i></div>
                <strong>Success!</strong>  Block Account Succesfull
                </div>";
    }else if (isset($_GET['err']) && $_GET['err'] == "bf"){
        echo  "<div id='mes' class='alert alert-danger alert-white rounded'>
                <button type='button' class='close' data-dismiss='alert' aria-hidden='true' id='cut'>×</button>
                <div class='icon'><i class='fa fa-times-circle'></i></div>
                <strong>Error!</strong> Block Account UnSuccesfull, Contact Admin
                </div>";
    }
}
?>
...
<?php
$sql_sub = "SELECT  * FROM teachers WHERE class = '$url_class'";
...
while($sub = mysqli_fetch_assoc($result_sub)){
    ...
    $acc_approval = $sub['acc_approval'];
        echo  "<tbody>".
            ...
            <input type='submit' class='btn' value='Open Full Profile' name='open_t'>
            </form>".
            "</td>".
            "<td>".
            "<form method='post'>".
            "<input type='hidden' name='id' value='"."$id"."'>
            <input type='submit' id='btn_ap-$i' class='btn_ap' value='Approved' name='app_t'>
            </form>".
            "</td>".
            "<td>".
            "<form method='post'>".
            "<input type='hidden' name='id' value='"."$id"."'>
            <input type='submit' id='btn_b-$i' class='btn_b' value='Block' name='app_b'>
            </form>".
            "</td>".
            "</tr>";
            $i++;
