...
if (isset($_POST['enter_sub'])) {
  ...
  $class = $_POST['class'];
  $sec = $_POST['Sec'];
  $sql="INSERT INTO `subjects` (`name`, `class`, `added_by`,section) VALUES ('$sub', '$url_class', '$name','$sec');";
  $result = mysqli_query($conn,$sql);

  if($result){
  header('location:addsub_en.php?err=ed&c='.$url_class.'');
  }else{
  header('location:addsub_en.php?err=ef&c='.$url_class.'');
  }
}
if (isset($_POST['del_sub'])) {
  $id = $_POST['id'];
echo $id;
  $sql="DELETE FROM `subjects` WHERE `subjects`.`sub_id` = '$id'";
  $result = mysqli_query($conn,$sql);

  if($result){
  header('location:addsub_en.php?err=ds&c='.$url_class.'');
  }else{
  header('location:addsub_en.php?err=df&c='.$url_class.'');
  }
 }
?>
<!DOCTYPE html>
...
<h1>Add Subject of Class <?php echo $url_class;?>:</h1><br>
Subject Name <span id="req">*</span><br>
<input class="data_st" type="text" name="sub" placeholder="Subject Name"><br>
<br>
Class <span id="req">*</span><br>
<input class="data_st" type="text" name="class" placeholder="Class"><br>
<br>
Section <span id="req">*</span><br>
<input class="data_st" type="text" name="sec" placeholder="Section"><br>
<br>
...
<table id="te">
  <thead>
   ...
   <tbody>
<?php
$sql_sub = "SELECT  * FROM subjects WHERE class = '$url_class'";
$result_sub = mysqli_query($conn,$sql_sub);
$i =1;
while($sub = mysqli_fetch_assoc($result_sub)){
    $class = $sub['class'];
    $name_admin = $sub['added_by'];
    $section = $sub['section'];
    $subject = $sub['name'];
    $id = $sub['sub_id'];
        echo"<tr>".
