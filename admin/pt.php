...
// End of profile_img
if (isset($_POST['open_pro'])) {
  $id = $_POST['id'];
  header('location:p_pro.php?s='.$id.'');
}
if (isset($_POST['app_s'])) {
  $id = $_POST['id'];
  $sql="UPDATE `parents` SET `acc_approval` = '1' WHERE `parent_id` = '$id';";
  $result = mysqli_query($conn,$sql);

  if($result){
  header('location:pt.php?err=ad');
  }else{
  header('location:pt.php?err=af');
  }
}
if (isset($_POST['app_b'])) {
  $id = $_POST['id'];
  $sql="UPDATE `parents` SET `acc_approval` = '0' WHERE `parent_id` = '$id';";
  $result = mysqli_query($conn,$sql);

  if($result){
  header('location:pt.php?err=bd');
  }else{
  header('location:pt.php?err=bf');
...
while($sub = mysqli_fetch_assoc($result_sub)){
    ...
    $img = $sub['user_image'];
        echo  "<tbody>".
            ...
            <input type='submit' id='btn' value='Open Full Profile' name='open_pro'>
            </form>".
            "</td>".
            "<td>".
            "<form method='post'>".
