...
if (isset($_POST['open_t'])) {
  $id = $_POST['id'];
  header('location:st_pro.php?s='.$id.'');
}
if (isset($_POST['app_s'])) {
  $id = $_POST['id'];
  $sql="UPDATE `students` SET `acc_approval` = '1' WHERE `roll_no` = '$id';";
  $result = mysqli_query($conn,$sql);

  if($result){
  header('location:studentlist.php?err=ad&c='.$url_class.'');
  }else{
  header('location:studentlist.php?err=af&c='.$url_class.'');
  }
}
if (isset($_POST['app_b'])) {
  $id = $_POST['id'];
  $sql="UPDATE `students` SET `acc_approval` = '0' WHERE `roll_no` = '$id';";
  $result = mysqli_query($conn,$sql);

  if($result){
  header('location:studentlist.php?err=bd&c='.$url_class.'');
  }else{
  header('location:studentlist.php?err=bf&c='.$url_class.'');
...
while($sub = mysqli_fetch_assoc($result_sub)){
    ...
    $acc_approval = $sub['acc_approval'];
        echo  "<tbody>".
            ...
            <input type='submit' id='btn' value='Open Full Profile' name='open_t'>
            </form>".
            "</td>".
            "<td>".
            "<form method='post'>".
            "<input type='hidden' name='id' value='"."$id"."'>
            <input type='submit' id='btn_ap-$i' class='btn_ap' value='Approved' name='app_s'>
            </form>".
            "</td>".
            "<td>".
            "<form method='post'>".
