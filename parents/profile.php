...
$sqlp_img = "select * from parents where username ='$p_id' and user_image <> '0'";
...
if(isset($_POST['submit_profile_info'])){
  ...
  $name = $_POST['p_name'];
  $occ = $_POST['p_occ'];
  $phone1 = $_POST['f_phone'];
  $phone2 = $_POST['f_phone_2'];
  $address = $_POST['address'];
  $paddress = $_POST['p_address'];


  $sql = "UPDATE `parents`
          SET `email` = '$email',`cnic` = '$cnic',`phone_1` = '$phone1',`phone_2` = '$phone2', `Address` = '$address',`parent_name` = '$name',`occupation` = '$occ' ,`p_Address` = '$paddress'
          WHERE `parent_id` = '$p_id'";
  $result = mysqli_query($conn,$sql);

  if($result){
  header('location:../parents/profile.php?err=updonep');
  }else{
  header('location:../parents/profile.php?err=upfailp');
  }
}
// update pass
if(isset($_POST['passreset'])){
  ...
  if($oldpass == $pass_p){
  $sql = "UPDATE `parents` SET `password` ='$newpass' WHERE `parent_id` = '$p_id'";
  $result = mysqli_query($conn,$sql);
  }
  if($result){
      header('location:../parents/profile.php?err=passdone');
  }else{
      header('location:../parents/profile.php?err=passno');
  }
}
?>
<!DOCTYPE html>
...
<?php
    if($p_img == 1){
      echo  "<img id='profileImage' src='assets/img/parent.jfif' id='profile_image' alt='Please Uploaded Photo'>
       <form id='imgformmob' action='../profile_img/profileupload.php?id=$p_id' method='POST' enctype='multipart/form-data'>
       <input type='file' name='image' value='Upload Image' class='fileToUpload-btn'>
       <h3 id='text' >Maximum 1 MB</h3>
       <button class='fileToUpload' name='parent' class=''>Upload Profile</button>
       </form>";
}else{
...
?>
    ...
    <input type="text" name="p_email" id="" class="data_st" value="<?php echo $email_p;?>" placeholder="Enter Email"> <br>
    <label>
    CNIC Number
    <span id="req">*</span>
    </label>
    <input type="text" name="p_cnic" id="" class="data_st" value="<?php echo $p_cnic;?>" placeholder="Enter CNIC"> <br>
    <label>
    Occupation
    <span id="req">*</span>
    </label>
    <input type="text" name="p_occ" id="" class="data_st" value="<?php echo $occ_p;?>" placeholder="Enter Father Occupation"> <br>
    <label>
    Phone Number-1
    <span id="req">*</span>
    </label>
    <input type="text" name="f_phone" id="" class="data_st" value="<?php echo $phone_1_p;?>" placeholder="Enter Phone Number-1"> <br>
    <label>
    Phone Number-2
    <span id="req">*</span>
    </label>
    <input type="text" name="f_phone_2" id="" class="data_st" value="<?php echo $phone_2_p;?>" placeholder="Enter Phone Number-2"> <br>
    <label>
    Current Address
    <span id="req">*</span>
    </label>
