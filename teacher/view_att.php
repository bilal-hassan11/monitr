...
<form method="post">
    <label class="label">Select Date<span class="text-danger ml-2">*</span></label>
        <input type="date" name="dateTaken" placeholder="Enter Date">
...
if(isset($_POST['view'])){
  ...
  // $query = "SELECT * FROM `attendance` WHERE class = '$class' and section = '$sec' and dateTaken = '$dateTaken' and subject = '$sub'";
  $query = "SELECT * 
  FROM attendance a JOIN students s 
  on a.roll_no = s.roll_no
  WHERE a.class='$class' and a.section = '$sec' and a.dateTaken = '$dateTaken' and a.subject = '$sub'";
  $rs = $conn->query($query);
  $num = mysqli_num_rows($rs);
  $sn=0;
  $status="";
  echo "<h2>Showing Attandence Of '".$dateTaken."' :</h2>";
  if($num > 0)
  { 
