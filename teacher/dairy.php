...
<?php 
    ...
    if (isset($_GET['err']) && $_GET['err'] == "npp"){
        echo  "<div id='mes' class='alert alert-danger alert-white rounded'>
        <button type='button' class='close' data-dismiss='alert' aria-hidden='true' id='cut'>×</button>
        <div class='icon'><i class='fa fa-times-circle'></i></div>
        <strong>Error!</strong> Updload Error, Contact Admin!!!
        </div>";
    }
...
?>
<form action='../dairy/dairyUpload.php?id=<?php echo $tid;?>' method='POST' enctype='multipart/form-data'>
<h1>Enter Dairy</h1>    
<label>
    dairy
    <span id="req">*</span>
    </label>
    <input class="data_st" type="text" name="dairy" placeholder="Enter Dairy"> <br>
    Class
    <span id="req">*</span>
    </label>
    ...
    <input class="data_st" type="hidden" name="tname" value="<?php echo $name; ?>"> <br>
...
<br>
<br>
<ul><h1 style="color: black;">Uploaded Assingment :</h1></ul>
<?php
$sql_sub = "SELECT `file`,`sub`,`dairy`,`class`,`section`,`date` FROM `dairy` WHERE class = '$class' and section = '$sec' and t_id = '$tid';";
...
while($row = mysqli_fetch_assoc($result_sub)){
  ...
  if($file !== 'none'){
    echo" <div class ='pdf'>
    <u><h2>Subject:  $sub</h2></u>
        <h5>Dairy: $dairy</h5><textarea id='txtbox' cols='50' rows='10'> $dairy </textarea>
        <h5>Class: $class</h5>
        <h5>Section: $sec</h5>
        <h5>Date: $date</h5>
        <h5>File:</h5>
        <embed src='$file' width='500' height='375' type='application/pdf' alt='No file Uploaded'><br>
        <h5>------------------------------------------------------------------------------------</h5>";
  }else{
    echo" <div class ='pdf'>
    <u><h2>Subject:  $sub</h2></u>
          <h5>Dairy: </h5><textarea id='txtbox' cols='50' rows='10'> $dairy </textarea>
          <h5>Class: $class</h5>
          <h5>Section: $sec</h5>
          <h5>Date: $date</h5>
          <h5>File: Not Uploaded</h5><br>
        <h5>------------------------------------------------------------------------------------</h5>";
  }
    }
    ?>
